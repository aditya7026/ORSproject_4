...
public class MarksheetListCtl extends BaseCtl {
    ...
    protected BaseBean populateBean(HttpServletRequest request){
    	log.debug("MarksheetListCtl Method populateBean start");
		...
		log.debug("MarksheetListCtl Method populateBean end");
    	return bean;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("MarksheetListCtl Method doGet start");
		MarksheetModel model = new  MarksheetModel();
		int pageNo = 1;
		int pageSize = DataUtility.getInt(PropertyReader.getValue("page.size"));
		List<MarksheetBean>list  = new ArrayList<MarksheetBean>();
		List<MarksheetBean>next = new ArrayList<MarksheetBean>();
		...
		log.debug("MarksheetListCtl Method doGet end");
		return;
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		log.debug("MarksheetListCtl Method doPost start");
		...
		try {
			if(op.equals(OP_NEW)){
				...
				log.debug("MarksheetListCtl Method doPost end");
				return;
			}
			...
			request.setAttribute("studentId", bean.getStudentId());
			request.setAttribute("rollNo", bean.getRollNo());
			request.setAttribute("studentName", bean.getName());
			ServletUtility.setList(list, request);
			ServletUtility.setPageNo(pageNo, request);
			ServletUtility.setPageSize(pageSize, request);
			ServletUtility.forward(getView(), request, response);
